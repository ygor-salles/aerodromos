...
app.get("/tsc", (req, res, next) => {
  pool
    .query(`SELECT tsc.type, tsc.latitude, tsc.longitude FROM public.tsc`)
...
app.get("/adhoc", (req, res, next) => {
  ...
  if(metar) {
    fields = fields.concat(["metar.date AS metar_date", "metar.message AS metar_message"]);
  }

  if (taf) {
    fields = fields.concat(["taf.date AS taf_date", "taf.message AS taf_message"]);
  }
